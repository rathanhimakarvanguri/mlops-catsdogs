...
- CD (.github/workflows/cd.yml): triggered after CI succeeds on main — pulls
  the image, deploys via Docker Compose, and runs scripts/smoke_test.sh (fails
  the pipeline if the health check or a live prediction fails).
...
are logged with MLflow: mlflow ui --port 5001
